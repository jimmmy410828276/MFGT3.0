...
public class MessageAdapter extends RecyclerView.Adapter <MessageAdapter.MessageHolder>{
    ...
    public MessageAdapter(ArrayList<Message> message, String senderImg, String receiverImg, Context context) {
    ...
    public void onBindViewHolder(@NonNull MessageHolder holder, int position) {
        holder.txtMessage.setText(message.get(position).getContet());

        ConstraintLayout constraintLayout = holder.ccll;

        if(message.get(position).getSender().equals(FirebaseAuth.getInstance().getCurrentUser().getEmail())){
